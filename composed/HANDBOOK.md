...
- **ico/penalty-schema** — basis: lm sourced from ICO (Information Commissioner's Office) real public fines (UK GDPR / Data Protection Act 2018 s157). Scaled to a subscriber turnover of 86,000,000.00 GBP, clamped at the statutory maximum max(cap, rate x turnover). FINALITY UNCHECKED: no example in payload version v3 carries a `status`, so whether any of the 2 figure(s) below is the FINAL collected penalty or a notice figure since reduced, set aside or never collected could not be looked at. From major 4 every example carries `status`, `final_as_of` and `litigation` (eco-system ticket 79 item 1). A named could-not-look. FREQUENCY UNSOURCED: this line annualises at (1, 2, 4) events/yr, which is this converter's editorial default and not a counted rate. Payload version v3 predates the publisher's `frequency` field, so no basis and no denominator could be read for it; from major 4 the basis belongs on regimes.uk-gdpr.violation_types.lower-tier.frequency.basis. A named could-not-look (eco-system ticket 79 item 3), never a bare number.
- **ico/penalty-schema supersede** — clock starts 2026-09-10; priced as of 2026-08-28. zero (as_of 2026-08-28 precedes the tag day 2026-09-10): the signed artefact's as-of is its newest signed input; only a re-composition --as-of a later day (the scheduled proposer's) grows this line
...
- **feeds/threat-register supersede** — clock starts 2026-09-10; priced as of 2026-08-28. zero (as_of 2026-08-28 precedes the tag day 2026-09-10): the signed artefact's as-of is its newest signed input; only a re-composition --as-of a later day (the scheduled proposer's) grows this line; the pinned checkout carries no directory for threat-register/v3.0.0, so the newer major's content is unread here: this line is priced from the signed tags alone, and no retirement to v3 is proposed until the feeds pin reaches a commit that carries it
...
- Regimes (3):
  ...
  - `threat-register` from feeds feed `threat-register` v2: GBP 19,558.55, 0 control(s) named
  - `forward-intel` from twin feed `forward-intel` 1.0.0: GBP 1,897,646.11, 0 control(s) named
...
- twin/forward-intel: unknown → isolated; retained residual unknown → GBP 37,952.92; delta unknown.
...
**7 field(s) this render looked for in the artefact and did not find.** Where a field is absent this page states nothing in its place — no default prose, no zero (ADR-0020: a missing instrument refuses; it is never invented).
...
Counted from the artefact: 5 publisher(s), 17 installed object(s), 17 recorded member(s), 9 price(s), 287 selected control(s), 285 hole(s), 2 recorded limit(s), 7 named absence(s).
