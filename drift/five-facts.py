#!/usr/bin/env python3
"""The five-fact sample: is driftwood's composed policy set in force, from signed sources?
...
## Sharing samples.jsonl with build ticket 64

ADR-0023's observation lane names `drift/samples.jsonl` and no other path, so a clock may write
nothing else and these records live in the same log as the hourly drift probe's. They do not mix:

  * every record here carries `"kind": "flux.five-facts/v1"`, which no probe sample carries;
  * `ts` and `reachable` are present so `twin/drift.py`'s loader accepts the line;
  * `reachable` is TRUE only when the sampled cluster is the one build ticket 64 declared
    (`kind-driftwood`) -- on any other cluster its three subjects were not sampled at all, so the
    run contributes a coverage hole and not a quiet stretch of no drift. That is the same rule the
    probe follows when it cannot reach the cluster, and it is why a CI run on an ephemeral cluster
    cannot inflate build ticket 64's coverage.
"""
...
# Build ticket 64's declared cluster. A sample taken anywhere else is not an observation of its
# three subjects -- see the module docstring.
WINDOW_CLUSTER = "kind-driftwood"
...
SAMPLER_EMAIL = "drift-sample@policy-as-versioned-driftwood.invalid"
...
def sources() -> list[dict]:
    ...
    driftwood-composed is the adopter's own signed tag, which is what the ResourceSet installs.
    ...
    for name, path, party, consumer in (
        ("driftwood-composed", "composed/composed-set.yaml", "driftwood", "resourceset"),
...
def ci_identity() -> dict:
    """The identity CI pins for driftwood's own tags. Falsifier 2 compares what the cluster
...
def inventory_ids(cluster: Cluster) -> dict[str, str]:
    ...
    sit on kind-driftwood, and no inventory anywhere lists them."""
    found: dict[str, str] = {}
...
def take_sample(context: str, cluster_name: str, ref: str | None,
    ...
    # The probe's own three subjects, but only where this IS build ticket 64's cluster.
    subjects: dict[str, str] = {}
    revision = ""
    top = cluster.get("-n", "flux-system", "get", "kustomization", "driftwood")
    if top:
        revision = str((top.get("status") or {}).get("lastAppliedRevision", ""))
    if cluster.reachable is None:
        cluster.get("get", "ns", "kube-system")
    if cluster.reachable and cluster_name == WINDOW_CLUSTER:
        for subject, args, key in (
            ("driftwood-live-version", ("-n", "driftwood", "get", "cm", "driftwood-live-version"), "policyVersion"),
            ("driftwood-nist-pin", ("-n", "driftwood", "get", "cm", "driftwood-nist-pin"), "catalogVersion"),
        ):
            got = cluster.get(*args)
            subjects[subject] = str((got or {}).get("data", {}).get(key, ""))
        ns = cluster.get("get", "ns", "driftwood")
        subjects["driftwood-namespace"] = str((ns or {}).get("metadata", {}).get("name", ""))
    ...
    for src in sources():
        ...
        records.append({
            "ts": stamp,
            # See the module docstring: true only on build ticket 64's own declared cluster.
            "reachable": bool(cluster.reachable) and cluster_name == WINDOW_CLUSTER,
            "reason": "" if cluster_name == WINDOW_CLUSTER else (
                f"this is a five-fact Flux sample taken on {cluster_name}, not "
                f"{WINDOW_CLUSTER}: build ticket 64's three subjects were not sampled by this run"),
            "kind": KIND,
...
def _identity_pin_wrong(party: str, regexp: str, ci: dict) -> str:
    ...
    driftwood's own is release.yml's constant, read here. A parent's release.yml is not in this
    ...
    """
    if party == "driftwood" and ci.get("regexp") and regexp != ci["regexp"]:
...
def selfcheck() -> int:
    ...
    assert inventory_id({"apiVersion": "v1", "kind": "ConfigMap",
                         "metadata": {"name": "c", "namespace": "driftwood"}}) \
        == "driftwood_c__ConfigMap"
    ...
    ci = {"regexp": r"^https://github\.com/policy-as-versioned-driftwood/driftwood/x$",
          "issuer": ACTIONS_ISSUER}
    src = {"party": "driftwood", "tag": "v1.1.0"}
    def live_with(**ann):
    ...
    # Where gitsign is on the PATH, gitsign_verifies RUNS and the falsifier has a real verdict.
    # Where it is not (shift-left.yml installs no gitsign; ticket 161's verify-cage-probe.sh was
    # the first thing to run this selfcheck there), the same call is a named could-not-look and
    # never `false`: the assertion says which of the two it saw, instead of demanding a verdict
    # from a tool the runner does not have.
    import shutil as _shutil
    if _shutil.which("gitsign"):
        assert ran_and_passed["fired"] in (True, False), \
            "where gitsign_verifies RAN, the falsifier has a real verdict"
    else:
        assert ran_and_passed["fired"] is None and "gitsign could not run" in ran_and_passed["why"], \
            "where gitsign is not installed, the falsifier is a could-not-look that names gitsign"
...
def main() -> int:
    ...
    take.add_argument("--context", default=os.environ.get("DRIFT_CONTEXT", "kind-driftwood"))
